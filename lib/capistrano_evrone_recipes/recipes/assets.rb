...
namespace :deploy do
  namespace :assets do
    ...
    DESC
    task :update_asset_mtimes, :on_no_matching_servers => :continue, :roles => assets_role, :except => { :no_release => true } do
      CapistranoEvroneRecipes::Util.ensure_changed_remote_dirs(self, "app/assets") do
    ...
    desc <<-DESC
      ...
      an existing release.
    DESC
    task :clean_expired, :on_no_matching_servers => :continue, :roles => assets_role, :except => { :no_release => true } do
    ...
    DESC
    task :rollback, :on_no_matching_servers => :continue, :roles => assets_role, :except => { :no_release => true } do
